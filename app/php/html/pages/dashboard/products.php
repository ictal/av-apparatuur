...
<section class='dashboard-content clearfix' ng-controller="productManager as pm" ng-init="pm.loadProducts()">
					<section><!-- Controller hier -->
						<section id='popup' class="popup" ng-class="pm.alert ? 'show' : 'invisible' ">
							<section id='popup1' class="inner-popup  "  ng-class="pm.alert ? 'scroll-up' : '' ">
								...
								<center><button href="?"class='btn btn-blue' ng-click='pm.deleteProduct()'>Ja</button><button ng-click='pm.dismiss($event)'class='btn btn-danger'>Nee</button></center>
							</section>
						</section>
					...
					<section class='notifications' >				
						...
						<table>
							<tr id='head' >
								<th width='35px'> <input type='checkbox' id='selected_products' name='selected_products' ng-click='pm.selectAll($event)'></th>
								<th width='25px'>id</th>
							...
							<tr ng-repeat="product in pm.products">
								<td ><input type='checkbox' id="{{product.id}}"ng-click="pm.addToList( product.id )" name='selected_products'></td>
								<td >{{ product.id }}</td>
					...
					<section class='notifications' ng-show="pm.showAdd">
						...
						<form action=<?php echo htmlspecialchars($_SERVER["PHP_SELF"] .'?p=products'); ?> method="post" enctype="multipart/form-data">
						<label style="margin-bottom: 9px; display: block;">
							<span style="border-top-width: 0px; height: 22px; width: 61px; padding-bottom: 0px; border-bottom-width: 12px; padding-right: 0px; border-right-width: 0px; margin-right: 28px; font-family: arial, sans-serif;"> img </span>
						...
						<label style="margin-bottom: 9px; display: block;">
							<span style="border-top-width: 0px; height: 22px; width: 61px; padding-bottom: 0px; border-bottom-width: 12px; padding-right: 0px; border-right-width: 0px; margin-right: 28px; font-family: arial, sans-serif;"> serial </span>
							
							<input type='text' name='product_serial_{{ key }}' placeholder="serial_number" ng-repeat='key in pm.NewArray( pm.aantal )'required>
						...
						<label style="margin-bottom: 9px; display: block;">
							<span style="border-top-width: 0px; height: 22px; width: 61px; padding-bottom: 0px; border-bottom-width: 12px; padding-right: 0px; border-right-width: 0px; margin-right: 28px; font-family: arial, sans-serif;"> description </span>
					...
					<section class='notifications' ng-show="pm.showEdit && pm.selectedProducts.length > 0 && pm.selectedProducts.length < 2">
						...
						<label style="margin-bottom: 9px; display: block;">
							<span style="border-top-width: 0px; height: 22px; width: 61px; padding-bottom: 0px; border-bottom-width: 12px; padding-right: 0px; border-right-width: 0px; margin-right: 28px; font-family: arial, sans-serif;"> naam</span>
							<input type='text' ng-model="pm.editAbleProduct.name" placeholder="{{ pm.getselectedProduct( 'name' ) }} ">
						</label>
						
						<label style="margin-bottom: 9px; display: block;">
							<span style="border-top-width: 0px; height: 22px; width: 61px; padding-bottom: 0px; border-bottom-width: 12px; padding-right: 0px; border-right-width: 0px; margin-right: 28px; font-family: arial, sans-serif;"> serial </span>
							<input type='text' ng-model="pm.editAbleProduct.serial_number" placeholder="{{ pm.getselectedProduct( 'serial_number' ) }} ">
						</label>
						
						<label style="margin-bottom: 9px; display: block;">
							<span style="border-top-width: 0px; height: 22px; width: 61px; padding-bottom: 0px; border-bottom-width: 12px; padding-right: 0px; border-right-width: 0px; margin-right: 28px; font-family: arial, sans-serif;"> description </span>
							<input type='text' ng-model="pm.editAbleProduct.description" placeholder="{{ pm.getselectedProduct( 'description' ) }} ">
						</label>
